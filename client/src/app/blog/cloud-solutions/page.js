...
export default function CloudSolutionsBlog() {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const router = useRouter();
  
  const blog = blogData['cloud-solutions'];
          ...
          <header className="bg-white/70 backdrop-blur-md border-b border-gray-200/50 px-3 sm:px-6 py-4 dark:bg-gray-950/70 dark:border-gray-800/50 shadow-lg">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2 sm:space-x-4 shrink-0">
                ...
                <Button size="sm" onClick={() => setIsModalOpen(true)} className="bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700">
                  Get Quote
                </Button>
              </div>
            </div>
            ...
            <article className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
                <div className="flex flex-wrap justify-center gap-2 mb-6">
                  {blog.tags.map((tag) => (
                    <span key={tag} className="px-3 py-1 text-sm font-medium bg-indigo-100 text-indigo-700 rounded-full dark:bg-indigo-900/30 dark:text-indigo-400">
                      {tag}
                    </span>
                  ))}
                </div>
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-6 bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 bg-clip-text text-transparent leading-tight">
                  {blog.title}
                </h1>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="prose prose-lg max-w-none mb-12">
                <div className="text-xl leading-relaxed text-gray-700 dark:text-gray-300 bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-950/20 dark:to-blue-950/20 p-8 rounded-2xl border border-gray-200/50 dark:border-gray-800/50">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }} className="text-center py-12 bg-gradient-to-r from-indigo-600 to-blue-600 rounded-3xl text-white mt-16">
                <Cloud className="h-16 w-16 mx-auto mb-6 opacity-80" />
                <h3 className="text-2xl sm:text-3xl font-bold mb-4">Ready to Scale in the Cloud?</h3>
                <p className="text-lg mb-8 max-w-2xl mx-auto opacity-90">{blog.content.cta}</p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                  <Button size="lg" variant="secondary" onClick={() => setIsModalOpen(true)} className="bg-white text-indigo-600 hover:bg-gray-100 px-8 py-4">
                    Get Free Quote<ArrowRight className="h-5 w-5 ml-2" />
                  </Button>
                  <Button size="lg" variant="outline" onClick={() => router.push('/portfolio')} className="border-white text-white hover:bg-white hover:text-indigo-600 px-8 py-4">
                    View Portfolio
                  </Button>
                </div>
              </motion.div>
