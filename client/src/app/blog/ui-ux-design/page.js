...
export default function UIUXDesignBlog() {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const router = useRouter();
  
  const blog = blogData['ui-ux-design'];
          ...
          <header className="bg-white/70 backdrop-blur-md border-b border-gray-200/50 px-3 sm:px-6 py-4 dark:bg-gray-950/70 dark:border-gray-800/50 shadow-lg">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2 sm:space-x-4 shrink-0">
                ...
                <Button size="sm" onClick={() => setIsModalOpen(true)} className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700">
                  Get Quote
                </Button>
              </div>
            </div>
            ...
            <article className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
                <div className="flex flex-wrap justify-center gap-2 mb-6">
                  {blog.tags.map((tag) => (
                    <span key={tag} className="px-3 py-1 text-sm font-medium bg-pink-100 text-pink-700 rounded-full dark:bg-pink-900/30 dark:text-pink-400">
                      {tag}
                    </span>
                  ))}
                </div>
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-6 bg-gradient-to-r from-pink-600 via-purple-600 to-blue-600 bg-clip-text text-transparent leading-tight">
                  {blog.title}
                </h1>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="prose prose-lg max-w-none mb-12">
                <div className="text-xl leading-relaxed text-gray-700 dark:text-gray-300 bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-950/20 dark:to-purple-950/20 p-8 rounded-2xl border border-gray-200/50 dark:border-gray-800/50">
                ...
                {blog.content.sections.map((section, index) => (
                  <motion.section key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + index * 0.1 }} className="prose prose-lg max-w-none">
                    <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-gray-900 dark:text-gray-100 flex items-center">
                      <div className="w-8 h-8 bg-gradient-to-r from-pink-500 to-purple-500 rounded-lg flex items-center justify-center mr-4">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }} className="text-center py-12 bg-gradient-to-r from-pink-600 to-purple-600 rounded-3xl text-white mt-16">
                <Palette className="h-16 w-16 mx-auto mb-6 opacity-80" />
                <h3 className="text-2xl sm:text-3xl font-bold mb-4">Ready to Transform Your Design?</h3>
                <p className="text-lg mb-8 max-w-2xl mx-auto opacity-90">{blog.content.cta}</p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                  <Button size="lg" variant="secondary" onClick={() => setIsModalOpen(true)} className="bg-white text-pink-600 hover:bg-gray-100 px-8 py-4">
                    Get Free Quote<ArrowRight className="h-5 w-5 ml-2" />
                  </Button>
                  <Button size="lg" variant="outline" onClick={() => router.push('/portfolio')} className="border-white text-white hover:bg-white hover:text-pink-600 px-8 py-4">
                    View Portfolio
                  </Button>
                </div>
              </motion.div>
